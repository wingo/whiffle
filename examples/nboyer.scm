...
(define (test-boyer alist term n)
...
(define if-constructor (symbol->symbol-record 'if))
(define false-term (translate-term '(f)))
(define true-term  (translate-term '(t)))
  ...
  (writeln
   (parallel nthreads
             (lambda (i)
               ...
               ;; Prevent Guile inlining.
               (set! alist alist)
               (set! term term)
               (test-boyer alist term n)))))
