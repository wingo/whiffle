...
(define (test test-proc test-args)
  ;;------------------------------------------------------------------------------
  ;;
  ;; The partial evaluator.

  (define (partial-evaluate proc args aggressive?)
    (peval (alphatize proc '()) args aggressive?))
  ...
  (define (peval proc args aggressive?)
    (simplify!
  ...
  (define (simplify! exp aggressive?) ;; simplify the expression 'exp' destructively (it
                ...
                (else
                 ...
                 (cond ((symbol? (car exp)) ;; is the operator position a var ref?
                              ...
                              (let ((proc (bound-expr (car exp) frame)))
                                (if (and (pair? proc)
                                    ...
                                    (let* ((args (arg-pattern (cdr exp)))
                                           (new-proc (peval proc args aggressive?))
                       ...
                       ((eq? (caar exp) 'lambda)
                        (set-car! where
                                  (list 'let
                                        (map2 (lambda (a b) (list a b)) (cadar exp) (cdr exp))
                                        (caddar exp)))
                        (s! where)))))))
  ...
  (define (binding-frame var env)
    (cond ((null? env) #f)
          ...
          (else
           (error #f "ill-formed environment"))))
  ...
  (list (try-peval example1 (list '(10 11) not-constant '1))
        ...
        (try-peval
         example8
         (list '(a b c d e f g h i j k l m n o p q r s t u v w x y z)))
        (partial-evaluate test-proc test-args #t)))

(lambda (nthreads size niters)
  (define (make-trees l)
    (match l
      (() '())
      ((head . tail)
       (cons (cons head (make-trees tail)) (make-trees tail)))))
  (define reverse-trees
    '(lambda (input)
       (letrec ((reverse-tree (lambda (x)
                                (if (pair? x) (reverse x '()) x)))
                (reverse (lambda (in result)
                           (if (pair? in)
                               (reverse (cdr in)
                                        (cons (reverse-tree (car in)) result))
                               result))))
         (reverse-tree input))))
  (define alphabet '(a b c d e f g h i j k l m n o p q r s t u v w x y z))
  (define input
    (let lp ((len (length alphabet))
             (input alphabet))
      (cond
       ((< len size) (lp (+ len len) (append input input)))
       ((eq? len size) (make-trees input))
       (else (lp (1- len) (cdr input))))))
  (define result
    (parallel nthreads
              (lambda (i)
                (let lp ((n 1))
                  (let ((res (test reverse-trees (list input))))
                    (if (< n niters)
                        (lp (1+ n))
                        res))))))
  (when (zero? niters) (writeln result)))
